class CollectionRecentlyViewed extends HTMLElement {
  ...
  loadRecentlyViewed(handles) {
    ...
    Promise.all(fetches).then((results) => {
      ...
      const arrowPrev = `<svg aria-hidden="true" focusable="false" class="h-10 w-10 text-seaweed-700 bg-white rounded-full block border-y-4 border-r-2 border-l-6 border-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>`;
      const arrowNext = `<svg aria-hidden="true" focusable="false" class="h-10 w-10 text-seaweed-700 bg-white rounded-full block border-y-4 border-r-2 border-l-6 border-white rotate-180" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>`;

      const sliderHtml = `<glide-slider
          class="glide relative -ml-1 gap-8 perView-4 lg:perView-3 md:perView-2"
        ...
        >
          ...
          <div class="glide__arrows" data-glide-el="controls">
            <button class="glide__arrow glide__arrow--left top-1/2" aria-label="Go to previous" data-glide-dir="<">${arrowPrev}</button>
            <button class="glide__arrow glide__arrow--right top-1/2" aria-label="Go to next" data-glide-dir=">">${arrowNext}</button>
          </div>
